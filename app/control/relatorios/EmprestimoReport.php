...
class EmprestimoReport extends TPage
{
    private $form; // form
    private $loaded;
    ...
    public function __construct()
    {
        ...
        $this->form->setFormTitle("Relatório de empréstimos");
        ...
        $exemplar_id->setMask('{codigo_barras} - {livro->titulo} ');
        ...
        $row4 = $this->form->addFields([new TLabel("Data prevista de devolução inicial:", null, '14px', null)],[$dt_previsao],[new TLabel("Data prevista de devolução final:", null, '14px', null)],[$dt_prevista_devolucao_final]);
        ...
        try
        {
            ...
            if ($objects)
            {
                ...
                if (!empty($tr))
                {
                    ...
                    $tr->addCell("Previsão de devolução", 'left', 'title');
                    ...
                    foreach ($objects as $object)
                    {
                        ...
                        $object->dt_previsao = call_user_func(function($value, $object, $row) 
                        {
                            if(!empty(trim($value)))
                            {
                                try
                        ...
                        }, $object->dt_previsao, $object, null);
                        ...
                        $tr->addCell($object->dt_previsao, 'left', $style);
